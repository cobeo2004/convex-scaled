...
const compose = (args: string) =>
  sh(`docker compose -f ../docker-compose.yml ${args}`);
const workers = async () =>
  (await compose("ps -q worker")).split("\n").filter(Boolean);
...
describe.runIf(process.env.FUNCTION_RUNNER === "remote")(
  "funrun failures",
  () => {
    test("mutations survive every worker being killed mid-load (committed at most once)", async () => {
      ...
      const errors = results
        ...
        .map(message);
      console.log(`kill mid-load: ${ok} committed, ${errors.length} failed`, [
        ...new Set(errors),
      ]);
      for (const e of errors)
        expect(e).toMatch(/overloaded|NoFunrunWorker|funrun worker/i);
    ...
    test("an action on a killed worker fails and is not re-run", async () => {
      ...
      const stats = await sh(
        `docker stats --no-stream --format '{{.ID}} {{.CPUPerc}}' ${(await workers()).join(" ")}`,
      );
      const busy = stats
        .split("\n")
        .map((l) => l.split(" "))
    ...
    test("graceful rolling restart under load: zero failed calls, actions drain", async () => {
      ...
      for (const id of await workers()) {
        ...
        actions.push({
          marker,
          done: http.action(api.actions.markedBurn, { marker, ms: 3_000 }),
        });
        await expect.poll(() => counter(marker), { timeout: 10_000 }).toBe(1);
      ...
      console.log(
        `rolling restart: ${writes} writes, ${failures.length} failures`,
        [...new Set(failures)],
      );
